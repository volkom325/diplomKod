...
def start(message):
    ...
    button10 = types.KeyboardButton("Вызов консультанта")
    button11 = types.KeyboardButton("Информация о боте")
...
def echo(message):
    if message.text == "Готовые ПК":
        ...
        bot.reply_to(message,"Для офисного компьюетра, который не будет загружен тяжелыми програмами рекомендую выбрать Radeon rx6400 4гб, ценной 8000 рублей"
                                              ...
                                              "\nа если вам требуется видеокарта, чтоб еще долго про нее не вспоминать и все идеально работало рекомендую поставить Radeon rx6900xt OC 16гб, ценной 35000 рублей")
        ...
        bot.reply_to(message, "Пожалуйста подождите, пока консультант подключается")
    elif message.text == "Информация о боте":
        bot.reply_to(message,"В мои функции входит:"
                             "\nВывод информации с рекомендациями о выбранной категории"
                             "\nТак же, если среди кнопок не будет найдено того, что вам нужно, вы можете нажать на кнопку вызов консультанта, в таком случае подключиться консультант для общения с вами")
    else:
